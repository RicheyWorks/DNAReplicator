...
public class ProteinService {
    ...
    private static class FoldingPathway {
        ...
        public JSONObject toJSON() {
            JSONObject json = new JSONObject();
            JSONArray bondsArray = new JSONArray();
            for (Bond bond : bonds) {
                bondsArray.put(bond.toJSON());
            }
    ...
    public ProteinService(List<Protein> proteins, ViewUpdater viewUpdater) {
        if (viewUpdater == null) {
            throw new IllegalArgumentException("ViewUpdater cannot be null");
        }
        if (proteins == null) {
            throw new IllegalArgumentException("Proteins list cannot be null");
        }
...
for (int i = newPopulation.size(); i < populationSize; i++) {
    ...
    private void saveSuccessfulPathways() {
        try (FileWriter writer = new FileWriter(Config.STATS_FILE_PATH + "_pathways.json")) {
            JSONArray pathwaysArray = new JSONArray();
            for (FoldingPathway pathway : successfulPathways) {
                pathwaysArray.put(pathway.toJSON());
            }
            writer.write(pathwaysArray.toString(4));
        } catch (IOException e) {
