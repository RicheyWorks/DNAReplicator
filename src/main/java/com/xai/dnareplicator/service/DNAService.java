...
public class DNAService {
    ...
    private List<DNAFragment> spliceHistory;
    private SimulationView view;
    private double mutationRate;
    private HashMap<String, DNAFragment> fragmentCache; // CLRS Chapter 11: Hash Tables for fragment lookup
    private HashMap<String, String> lcsCache; // CLRS Chapter 11: Cache for LCS results

    public DNAService(SimulationView view) {
        if (view == null) {
    ...
    public void toggleSelection(DNAFragment fragment) {
        ...
        int selectedCount = (int) dnaFragments.stream().filter(DNAFragment::isSelected).count();
    ...
    public void spliceDNA() {
        List<DNAFragment> selectedFragments = dnaFragments.stream()
        ...
        dnaFragments.remove(fragment1);
        dnaFragments.remove(fragment2);
    ...
    public List<DNAFragment> sortFragmentsByLength() {
        return sortFragmentsByLength(new ArrayList<>(dnaFragments));
    }
    ...
    public void undoSplice() {
        ...
        dnaFragments.add(fragment1);
        dnaFragments.add(fragment2);
        try {
        ...
        if (!dnaFragments.isEmpty()) {
            DNAFragment spliced = dnaFragments.get(dnaFragments.size() - 1);
            dnaFragments.remove(spliced);
        ...
        proteins.clear();
        view.clearAll();
        for (DNAFragment fragment : dnaFragments) {
            if (fragment != null) {
